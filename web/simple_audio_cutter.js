...
const TARGET_CLASS = "CF_SimpleAudioCutter";   // 仅修改此行
...
function hideWidget(widget) {
    if (!widget || widget.__ltxMyHidden) return;
    widget.__ltxMyOriginalType = widget.type;
    widget.__ltxMyOriginalComputeSize = widget.computeSize;
    widget.__ltxMyOriginalSerializeValue = widget.serializeValue;
    ...
    if (targets.length) {
        widget.__ltxMyElements = targets.map((target) => ({ target, cssText: target.style.cssText }));
        for (const { target } of widget.__ltxMyElements) {
...
function buildEditor(node) {
    if (node.__ltxMyWaveformEditor) return node.__ltxMyWaveformEditor;

    const container = document.createElement("div");
    container.className = "ltx-my-audio-waveform-editor";
    ...
    help.textContent = "点击波形可定位播放头（会自动吸附到 1 秒网格）。点“播放”试听。可在当前播放头位置添加标记。开启“跳过初始段”后，第一个标记只作为起点；开启“包含尾部段”后，会把最后一个标记到音频结尾也算成最后一段。";
    ...
    node.__ltxMyWaveformEditor = state;
    ...
    const loadWaveform = async () => {
        ...
        try {
            const waveformUrl = api.apiURL(`/simpleaudio/audio-waveform?audio_file=${encodeURIComponent(audioFile)}&bins=1400`);
    ...
    audio.addEventListener("play", () => { playButton.textContent = "Pause"; render(); });
...
app.registerExtension({
    name: "SimpleAudioCutter.WaveformEditor",
